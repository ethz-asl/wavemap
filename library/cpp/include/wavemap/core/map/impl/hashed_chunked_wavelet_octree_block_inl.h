...
template <typename CellDataT>
bool HashedChunkedWaveletOctreeBlockT<CellDataT>::empty() const {
  return ndtree_.empty() && OccupancyClassifier::isUnobserved(
                                chunked_detail::getOccupancy(root_scale_coefficient_));
}

template <typename CellDataT>
FloatingPoint HashedChunkedWaveletOctreeBlockT<CellDataT>::getTimeSinceLastUpdated()
    const {
  return time::to_seconds<FloatingPoint>(Time::now() - last_updated_stamp_);
}

template <typename CellDataT>
FloatingPoint HashedChunkedWaveletOctreeBlockT<CellDataT>::getCellValue(
    const OctreeIndex& index) const {
  return chunked_detail::getOccupancy(getVoxelValue(index));
}

template <typename CellDataT>
CellDataT HashedChunkedWaveletOctreeBlockT<CellDataT>::getVoxelValue(
    const OctreeIndex& index) const {
...
template <typename CellDataT>
void HashedChunkedWaveletOctreeBlockT<CellDataT>::threshold() {
  ProfilerZoneScoped;
...
template <typename CellDataT>
void HashedChunkedWaveletOctreeBlockT<CellDataT>::prune() {
  ProfilerZoneScoped;
  if (getNeedsPruning()) {
    threshold();
...
template <typename CellDataT>
void HashedChunkedWaveletOctreeBlockT<CellDataT>::clear() {
  ProfilerZoneScoped;
...
template <typename CellDataT>
void HashedChunkedWaveletOctreeBlockT<CellDataT>::setCellValue(
    const OctreeIndex& index, FloatingPoint new_value) {
  CellDataT voxel = getVoxelValue(index);
  chunked_detail::setOccupancy(voxel, new_value);
  setVoxelValue(index, voxel);
}

template <typename CellDataT>
void HashedChunkedWaveletOctreeBlockT<CellDataT>::setVoxelValue(
    const OctreeIndex& index, const CellDataT& new_value) {
  setNeedsPruning();
  setNeedsThresholding();
  setLastUpdatedStamp();
  const MortonIndex morton_code = convert::nodeIndexToMorton(index);
  std::vector<typename OctreeType::NodeRefType> ancestors;
  ...
  for (int parent_height = tree_height_; index.height + 1 < parent_height;
    ...
    const NdtreeIndexRelativeChild child_index =
        OctreeIndex::computeRelativeChildIndex(morton_code, parent_height);
    typename OctreeType::NodeRefType current_parent = ancestors.back();
  ...
  for (int parent_height = index.height + 1; parent_height <= tree_height_;
    ...
    const NdtreeIndexRelativeChild child_index =
        OctreeIndex::computeRelativeChildIndex(morton_code, parent_height);
    typename OctreeType::NodeRefType current_node = ancestors.back();
    ancestors.pop_back();
    coefficients =
...
template <typename CellDataT>
void HashedChunkedWaveletOctreeBlockT<CellDataT>::addToCellValue(
    const OctreeIndex& index, FloatingPoint update) {
  addToVoxelValue(index, CellDataT{update});
}

template <typename CellDataT>
void HashedChunkedWaveletOctreeBlockT<CellDataT>::addToVoxelValue(
...
template <typename CellDataT>
void HashedChunkedWaveletOctreeBlockT<CellDataT>::forEachLeaf(
  ...
  forEachVoxelLeaf(
      ...
      [&visitor_fn](const OctreeIndex& node_index,
        ...
        visitor_fn(node_index, chunked_detail::getOccupancy(voxel_value));
      },
      termination_height);
}
...
void HashedChunkedWaveletOctreeBlockT<CellDataT>::forEachVoxelLeaf(
...
template <typename CellDataT>
void HashedChunkedWaveletOctreeBlockT<CellDataT>::recursiveThreshold(
  ...
  for (NdtreeIndexRelativeChild child_idx = 0;
    ...
    if (auto child_node = node.getChild(child_idx); child_node) {
      ...
      chunked_detail::thresholdCellData(child_scale, min_log_odds_, max_log_odds_,
                                threshold_config_);
    }
  }
...
template <typename CellDataT>
void HashedChunkedWaveletOctreeBlockT<CellDataT>::recursivePrune(
    typename OctreeType::NodeRefType node) {
  ...
  for (NdtreeIndexRelativeChild child_idx = 0;
    ...
    if (typename OctreeType::NodePtrType child_node = node.getChild(child_idx);
      ...
      if (!child_node->hasAtLeastOneChild() &&
          !chunked_detail::detailsHaveNonzeroData<CellDataT>(child_node->data(),
                                                     pruning_config_)) {
