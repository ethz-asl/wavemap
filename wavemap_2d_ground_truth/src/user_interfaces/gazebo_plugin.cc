...
bool Wavemap2DGroundTruthPlugin::serviceCallback(
  ...
  CHECK(nh_private_.getParam("resolution", resolution))
      << "ROS param " << nh_private_.resolveName("resolution")
      << " must be set.";
  CHECK(nh_private_.getParam("slice_height", slice_height))
      << "ROS param " << nh_private_.resolveName("slice_height")
      << " must be set.";
            ...
            if (mesh_manager) {
              ...
              for (uint submesh_id = 0;
                ...
                if (submesh.GetPrimitiveType() != common::SubMesh::TRIANGLES) {
                  LOG(ERROR) << "Encountered a mesh with type "
  ...
  LOG(INFO) << "Saving occupancy grid to file: " << request.file_path;
