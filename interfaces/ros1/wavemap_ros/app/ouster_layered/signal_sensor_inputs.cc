...
void registerSensorInputs(RosServer& server,
  ...
  // The bag's Ouster profile is SIG16. Preserve its full sensor-defined range
  // in normalized storage rather than fitting bounds to this recording.
...
int main(int argc, char** argv) {
  ...
  wavemap::RosServer server(
      ...
      [](wavemap::RosServer& server, ros::NodeHandle private_nh) {
        namespace map = wavemap::examples::signal_map;
        map::ServerBuilder::install(server, private_nh, server.worldFrame(),
                                    map::makeMapConfig(),
                                    map::registerSensorInputs);
        ROS_INFO(
            "Installed occupancy+reflectivity+signal benchmark application "
            "(E).");
      });
