...
namespace wavemap::examples::occupancy_map::chunked {
...
int main(int argc, char** argv) {
  ...
  wavemap::RosServer server(
      ...
      [](wavemap::RosServer& server, ros::NodeHandle private_nh) {
        namespace map = wavemap::examples::occupancy_map::chunked;
        map::ServerBuilder::install(
            server, private_nh, server.worldFrame(),
            wavemap::examples::occupancy_map::makeMapConfig<map::Definition>(),
            map::registerSensorInputs);
        ROS_INFO("Installed chunked layered occupancy-only benchmark.");
      });
