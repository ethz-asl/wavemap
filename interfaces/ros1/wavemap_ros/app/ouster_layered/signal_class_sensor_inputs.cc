...
namespace wavemap::examples::signal_map::signal_class_map {

using Definition = ClassDefinition;
using RosConverter =
...
void registerSensorInputs(RosServer& server,
  ...
  server.bindGeometricClass<ClassLayer>(
      extension, class_config, endpoint_range);
}
...
int main(int argc, char** argv) {
  ...
  wavemap::RosServer server(
      ...
      [](wavemap::RosServer& server, ros::NodeHandle private_nh) {
        namespace map = wavemap::examples::signal_map::signal_class_map;
        ...
        ROS_INFO(
            "Installed occupancy+reflectivity+signal+class benchmark "
            "application (G) with discrete block height %d.",
            discrete_block_height);
      });
