...
class VolumetricDataStructure2DTest : public FixtureBase {
 protected:
...
TYPED_TEST_SUITE(VolumetricDataStructure2DTest, VolumetricDataStructureTypes, );

TYPED_TEST(VolumetricDataStructure2DTest, InitializationAndClearing) {
...
TYPED_TEST(VolumetricDataStructure2DTest, Pruning) {
  std::unique_ptr<VolumetricDataStructure2D> map_base_ptr =
      std::make_unique<TypeParam>(TestFixture::getRandomMinCellWidth());
  const size_t empty_map_memory_usage = map_base_ptr->getMemoryUsage();
...
TYPED_TEST(VolumetricDataStructure2DTest, MinMaxIndexGetters) {
...
TYPED_TEST(VolumetricDataStructure2DTest, InsertionAndLeafVisitor) {
  constexpr int kNumRepetitions = 3;
  for (int i = 0; i < kNumRepetitions; ++i) {
    // Create a random map
    std::unique_ptr<VolumetricDataStructure2D> map_base_ptr =
