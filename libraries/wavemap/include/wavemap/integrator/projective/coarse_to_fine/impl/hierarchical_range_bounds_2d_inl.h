...
inline Bounds<FloatingPoint> HierarchicalRangeBounds2D::getBounds(
  ...
  if (index.height == 0) {
    const FloatingPoint range_image_value =
        range_image_->getRange(index.position);
    return {range_image_value, range_image_value};
  } else {
    return {lower_bound_levels_[index.height - 1].getRange(index.position),
            upper_bound_levels_[index.height - 1].getRange(index.position)};
  }
}
  ...
  if (((top_right_child_idx_unwrapped - bottom_left_child_idx).array().abs() <=
    ...
    if (bottom_left_child_idx.x() == top_right_child_idx_unwrapped.x() ||
      ...
      if (min_level_up == 0) {
        return {
            std::min(
                {valueOrInit(range_image_->getRange(bottom_left_image_idx),
                             kUnknownRangeImageValueLowerBound),
                 valueOrInit(range_image_->getRange({bottom_left_image_idx.x(),
        ...
        return {
            std::min({lower_bound_levels_[child_height_idx].getRange(
                          bottom_left_child_idx),
                      lower_bound_levels_[child_height_idx].getRange(
    ...
    return {
        std::min({lower_bound_levels_[parent_height_idx].getRange(
                      bottom_left_parent_idx),
    ...
    for (const Index2D& idx :
      ...
      if ((min_child_idx.array() < previous_level_dims.array()).all()) {
        const FloatingPoint r00 = valueOrInit(
            previous_level.getRange({min_child_idx.x(), min_child_idx.y()}),
            init, level_idx);
        if (max_child_idx.x() < previous_level_dims.x()) {
          const FloatingPoint r10 = valueOrInit(
              previous_level.getRange({max_child_idx.x(), min_child_idx.y()}),
              init, level_idx);
          ...
          if (max_child_idx.y() < previous_level_dims.y()) {
            const FloatingPoint r01 = valueOrInit(
                previous_level.getRange({min_child_idx.x(), max_child_idx.y()}),
                init, level_idx);
            const FloatingPoint r11 = valueOrInit(
                previous_level.getRange({max_child_idx.x(), max_child_idx.y()}),
                init, level_idx);
            ...
            const FloatingPoint r11 =
                valueOrInit(previous_level.getRange({max_child_idx.x(), 0}),
                            init, level_idx);
            const FloatingPoint second_col_reduced =
                reduction_functor(r01, r11);
            current_level.getRange(idx) =
            ...
            current_level.getRange(idx) =
                reduction_functor(first_col_reduced, init);
          }
          ...
          current_level.getRange(idx) = reduction_functor(r00, init);
        }
      } else {
        current_level.getRange(idx) = init;
      }
    }
  }
  return pyramid;
