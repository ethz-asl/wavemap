#ifndef WAVEMAP_INTEGRATOR_PROJECTIVE_COARSE_TO_FINE_IMPL_HIERARCHICAL_RANGE_BOUNDS_2D_INL_H_
#define WAVEMAP_INTEGRATOR_PROJECTIVE_COARSE_TO_FINE_IMPL_HIERARCHICAL_RANGE_BOUNDS_2D_INL_H_
...
inline Bounds<FloatingPoint> HierarchicalRangeBounds2D::getBounds(
  ...
  if (bottom_left_image_idx == top_right_image_idx) {
    const auto range = range_image_->at(bottom_left_image_idx);
  ...
  if (((top_right_child_idx_unwrapped - bottom_left_child_idx).array().abs() <=
    ...
    if (bottom_left_child_idx.x() == top_right_child_idx_unwrapped.x() ||
      ...
      if (min_level_up == 0) {
        return {std::min(valueOrInit(range_image_->at(bottom_left_image_idx),
                                     kUnknownRangeImageValueLowerBound),
                         valueOrInit(range_image_->at(top_right_image_idx),
                                     kUnknownRangeImageValueLowerBound)),
                std::max(valueOrInit(range_image_->at(bottom_left_image_idx),
        ...
        return {
            std::min(
                lower_bound_levels_[child_height_idx].at(bottom_left_child_idx),
                lower_bound_levels_[child_height_idx].at(top_right_child_idx)),
            std::max(
                upper_bound_levels_[child_height_idx].at(bottom_left_child_idx),
                upper_bound_levels_[child_height_idx].at(top_right_child_idx))};
      }
      ...
      return {
          lower_bound_levels_[parent_height_idx].at(bottom_left_parent_idx),
          upper_bound_levels_[parent_height_idx].at(bottom_left_parent_idx)};
    } else {
      // Check all four children at min_level_up
      if (min_level_up == 0) {
        return {
            std::min({valueOrInit(range_image_->at(bottom_left_image_idx),
                                  kUnknownRangeImageValueLowerBound),
                      valueOrInit(range_image_->at({bottom_left_image_idx.x(),
                                                    top_right_image_idx.y()}),
                                  kUnknownRangeImageValueLowerBound),
                      valueOrInit(range_image_->at({top_right_image_idx.x(),
                                                    bottom_left_image_idx.y()}),
                                  kUnknownRangeImageValueLowerBound),
                      valueOrInit(range_image_->at(top_right_image_idx),
                                  kUnknownRangeImageValueLowerBound)}),
            std::max({valueOrInit(range_image_->at(bottom_left_image_idx),
        ...
        return {
            std::min({lower_bound_levels_[child_height_idx].at(
                      ...
                      lower_bound_levels_[child_height_idx].at(
                          top_right_child_idx)}),
            std::max({upper_bound_levels_[child_height_idx].at(
                          bottom_left_child_idx),
        ...
        std::min(
            {lower_bound_levels_[parent_height_idx].at(bottom_left_parent_idx),
             lower_bound_levels_[parent_height_idx].at(
                 {bottom_left_parent_idx.x(), top_right_parent_idx.y()}),
             lower_bound_levels_[parent_height_idx].at(
                 {top_right_parent_idx.x(), bottom_left_parent_idx.y()}),
             lower_bound_levels_[parent_height_idx].at(top_right_parent_idx)}),
        std::max(
            {upper_bound_levels_[parent_height_idx].at(bottom_left_parent_idx),
             ...
             upper_bound_levels_[parent_height_idx].at(
                 {top_right_parent_idx.x(), bottom_left_parent_idx.y()}),
             upper_bound_levels_[parent_height_idx].at(top_right_parent_idx)})};
  }
}
...
std::vector<Image<>> HierarchicalRangeBounds2D::computeReducedPyramid(
    const Image<>& range_image, BinaryFunctor reduction_functor,
    FloatingPoint init) {
  ...
  for (int level_idx = 0; level_idx < max_num_halvings; ++level_idx) {
    ...
    for (const Index2D& idx :
      ...
      if ((min_child_idx.array() < previous_level_dims.array()).all()) {
        const FloatingPoint r00 = valueOrInit(
            previous_level.at({min_child_idx.x(), min_child_idx.y()}), init,
            level_idx);
        if (max_child_idx.x() < previous_level_dims.x()) {
          const FloatingPoint r10 = valueOrInit(
              previous_level.at({max_child_idx.x(), min_child_idx.y()}), init,
              level_idx);
          ...
          if (max_child_idx.y() < previous_level_dims.y()) {
            const FloatingPoint r01 = valueOrInit(
                previous_level.at({min_child_idx.x(), max_child_idx.y()}), init,
                level_idx);
            const FloatingPoint r11 = valueOrInit(
                previous_level.at({max_child_idx.x(), max_child_idx.y()}), init,
                level_idx);
            ...
            const FloatingPoint r01 = valueOrInit(
                previous_level.at({min_child_idx.x(), 0}), init, level_idx);
            const FloatingPoint r11 = valueOrInit(
                previous_level.at({max_child_idx.x(), 0}), init, level_idx);
            const FloatingPoint second_col_reduced =
                reduction_functor(r01, r11);
            current_level.at(idx) =
                reduction_functor(first_col_reduced, second_col_reduced);
...
#endif  // WAVEMAP_INTEGRATOR_PROJECTIVE_COARSE_TO_FINE_IMPL_HIERARCHICAL_RANGE_BOUNDS_2D_INL_H_
