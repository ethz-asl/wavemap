...
TEST_F(MeasurementModelTest, DISABLED_ContinuousVolumetricLogOddsModel) {
  ...
  for (int i = 0; i < kNumRepetitions; ++i) {
    ...
    config.range_sigma =
        random_number_generator_->getRandomRealNumber(1e-3f, 5e-2f);
    const ContinuousVolumetricLogOdds model(config);

    const Point3D W_start_point = getRandomPoint<3>();
    const Point3D W_end_point = W_start_point + getRandomTranslation<3>();

    const Point3D W_t_end_point_start_point = W_end_point - W_start_point;
    ...
    const Index3D bottom_left_idx = model.getBottomLeftUpdateIndex(
        W_start_point, W_end_point, beam_length, min_cell_width_inv);
    const Index3D top_right_idx = model.getTopRightUpdateIndex(
        W_start_point, W_end_point, beam_length, min_cell_width_inv);

    constexpr IndexElement kIndexPadding = 10;
    const Index3D padded_bottom_left_idx =
        bottom_left_idx - Index3D::Constant(kIndexPadding);
    const Index3D padded_top_right_idx =
        top_right_idx + Index3D::Constant(kIndexPadding);
    ...
    for (const auto& index : grid) {
      ...
      const Point3D W_t_cell_center_end_point = W_cell_center - W_start_point;
...
TEST_F(MeasurementModelTest, Constant1DLogOddsModel) {
  constexpr int kNumRepetitions = 100;
  for (int i = 0; i < kNumRepetitions; ++i) {
    const Point3D start_point = getRandomPoint<3>();
    const Point3D end_point = start_point + getRandomTranslation<3>();
    const FloatingPoint min_cell_width = getRandomMinCellWidth();
    const FloatingPoint min_cell_width_inv = 1.f / min_cell_width;

    Constant1DLogOdds model(min_cell_width);
    model.setStartPoint(start_point);
    model.setEndPoint(end_point);
    const Index3D start_point_index =
        convert::pointToNearestIndex(start_point, min_cell_width_inv);
    const Index3D end_point_index =
        convert::pointToNearestIndex(end_point, min_cell_width_inv);

    const Index3D bottom_left_idx = start_point_index.cwiseMin(end_point_index);
    const Index3D top_right_idx = start_point_index.cwiseMin(end_point_index);
    const Index3D model_bottom_left_idx = model.getBottomLeftUpdateIndex();
    const Index3D model_top_right_idx = model.getTopRightUpdateIndex();
    EXPECT_TRUE(
